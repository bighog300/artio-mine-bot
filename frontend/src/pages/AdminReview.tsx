  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
            ...
            {filtered.map((artist) => (
              ...
              >
                <span>
                  ...
                  <span className="text-xs text-muted-foreground">Completeness: {artist.completeness_score} · Missing: {artist.missing_fields.length}</span>
                </span>
              </Button>
            ))}
          </div>
        ...
        <div className="col-span-2 bg-card rounded border p-4 space-y-4">
          {!artistDetail.data && <div className="text-muted-foreground text-sm">Select an artist to review.</div>}
          {artistDetail.data && (
            <>
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-semibold">{artistDetail.data.title}</h2>
                  <p className="text-sm text-muted-foreground">Completeness: {artistDetail.data.completeness_score}</p>
                </div>
              ...
              <section>
                ...
                <div className="text-sm text-muted-foreground">{artistDetail.data.missing_fields.join(", ") || "None"}</div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-3">
                  ...
                  {Object.keys(artistDetail.data.conflicts).length === 0 && <div className="text-sm text-muted-foreground">No conflicts pending.</div>}
                </div>
              </section>
            </>
          )}
